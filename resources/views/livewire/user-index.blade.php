<main x-data="{ modalOpen: false }"
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        ...
        {{-- زر فتح مودال الإضافة وتصفير الحقول --}}
                ...
                <tbody class="divide-y divide-gray-100 text-sm text-gray-700">
                    @forelse($users as $user)
                        <tr class="hover:bg-gray-50/40 transition">
                            ...
                            {{-- أزرار العمليات (التعديل والحذف والاستعادة عبر ID) --}}
                            <td class="p-4 text-center">
                                <div class="flex items-center justify-center gap-1.5">
                                    @if(!$showDeleted)
                                        {{-- زر التعديل يحقن البيانات في المودال ويفتحه عبر الـ Event --}}
                                        ...
                                        {{-- زر الصلاحيات --}}
                                        <a href="{{ route('users.permissions', $user->id) }}" wire:navigate class="p-1.5 text-purple-600 hover:bg-purple-50 rounded-lg transition">
                                    ...
                                    @else
                                        {{-- زر استعادة المستخدم المؤرشف --}}
                                        <button wire:click="restoreUser({{ $user->id }})" class="p-1.5 text-green-600 hover:bg-green-50 rounded-lg transition">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-4.5 w-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 1121.212 9H19" /></svg>
                                        </button>
                                    @endif
                                </div>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="p-8 text-center text-gray-400 bg-gray-50/50">لا توجد نتائج.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    {{-- نافذة المودال المنبثقة (إضافة / تعديل) --}}
    <div x-show="modalOpen"
        ...
        <div @click.away="modalOpen = false; $wire.resetInputFields()"
            ...
            {{-- رأس المودال (يتغير العنوان ديناميكياً بحسب الكلاس) --}}
            ...
            {{-- فوروم إرسال البيانات (يرتبط بدالة save لتوجيه المسار) --}}
            <form wire:submit.prevent="save" class="p-6 space-y-4 overflow-y-auto flex-1 text-xs">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {{-- حقل كلمة المرور (ملاحظة ديناميكية تناسب التعديل والإضافة) --}}
                    <div>
                    ...
                    {{-- حالة المستخدم --}}
                    <div>
                        <label class="block font-medium text-gray-700 mb-1">حالة المستخدم</label>
                        ...
                        @error('active') <span class="text-red-500 text-[11px] mt-1 block">{{ $message }}</span> @enderror
                    </div>
                    {{-- مستوى العرض --}}
                    <div>
                        <label class="block font-medium text-gray-700 mb-1">مستوى العرض</label>
                        <select wire:model="view" class="w-full border border-gray-300 rounded-lg p-2 bg-white outline-none focus:border-indigo-500">
                            <option value="1">محلية</option>
                            <option value="2">شعبة</option>
                            <option value="3">مسجد</option>
                        </select>
                        @error('view') <span class="text-red-500 text-[11px] mt-1 block">{{ $message }}</span> @enderror
                    </div>
                    {{-- الاقسام --}}
                    <div>
                        <label class="block font-medium text-gray-700 mb-1">الشعبة</label>
                        <select wire:model="department_id" class="w-full border border-gray-300 rounded-lg p-2 bg-white outline-none focus:border-indigo-500">
                            @foreach ($departments as $department )
                            <option value="{{ $department->id }}">{{ $department->name }}</option>
                            @endforeach
                        </select>
                        @error('department_id') <span class="text-red-500 text-[11px] mt-1 block">{{ $message }}</span> @enderror
                    </div>
                    {{-- المساجد --}}
                    <div>
                        <label class="block font-medium text-gray-700 mb-1">الشعبة</label>
                        <select wire:model="mosque_id" class="w-full border border-gray-300 rounded-lg p-2 bg-white outline-none focus:border-indigo-500">
                            @foreach ($mosques as $mosque )
                            <option value="{{ $mosque->id }}">{{ $mosque->name }}</option>
                            @endforeach
                        </select>
                        @error('mosque_id') <span class="text-red-500 text-[11px] mt-1 block">{{ $message }}</span> @enderror
                    </div>
                </div>
                ...
                <div>
                    ...
                    @error('address') <span class="text-red-500 text-[11px] mt-1 block">{{ $message }}</span> @enderror
                </div>

                {{-- أزرار التحكم بالنموذج الأسفل (نص الزر يتغير بحسب نوع العملية) --}}
